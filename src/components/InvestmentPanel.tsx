import { INDEX_RENDIMENT_MIN, INDEX_RENDIMENT_RANG, PAS_PLA } from '../domain/constants'
import {
  benestarOciAnual,
  costVidaAnual,
  defaultPlaInversio,
    ...
    <div className="rounded-2xl bg-slate-800/70 p-5 ring-1 ring-slate-700/50">
      <div className="mb-1 flex items-baseline justify-between">
        ...
        <span className="text-sm text-slate-400">
          {t('pla.income')}: {formatEuros(income)}/any
        </span>
      </div>
      ...
      <div className="mt-3 space-y-1 border-t border-slate-700 pt-3 text-sm">
        ...
        {benestar <= 0 && (
          <p className="text-xs text-amber-400/80">
            {t('pla.benestar.min', { min: formatEuros(minOci) })}
          </p>
        )}
        ...
        {desgravacio > 0 && (
          <p className="text-xs text-emerald-300/90">
            🧾 {t('pla.notaPensions', { amount: formatEuros(desgravacio) })}
          </p>
        )}
        <div className="flex justify-between">
